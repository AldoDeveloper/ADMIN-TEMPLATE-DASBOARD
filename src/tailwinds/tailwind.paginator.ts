    ...
    paginator: {
        root: {
            className: classNames(
                ...
                'bg-white text-gray-500 border-0 px-2 py-2 rounded-md',
        ...
        pagebutton: ({ context }: any) => ({
            className: classNames(
                'relative inline-flex items-center justify-center user-none overflow-hidden leading-none',
                'border-0 text-gray-500 min-w-[1.5rem] min-h-[1.5rem] mx-[0.8rem] rounded-full',
                'transition duration-200',
                'dark:border-blue-300 dark:text-white', // Dark Mode
                'focus:outline-none focus:outline-offset-0 focus:shadow-[0_0_0_0.2rem_rgba(191,219,254,1)]', // Focus
                {
                    'bg-slate-200 border-blue-50 text-blue-700 dark:bg-slate-700': context.active
                }
            )
        }),
        rowperpagedropdown: {
        ...
        jumptopageinput: {
            ...
            input: {
                className: classNames(
                    'font-sans text-base text-gray-600 p-1 m-0 rounded-md apperance-none',
        ...
        jumptopagedropdown: {
            ...
            item: ({ context }: any) => ({
                className: classNames(
                    'relative font-normal cursor-pointer space-nowrap overflow-hidden',
                    'm-0 py-3 px-2 border-none text-gray-600 rounded-none',
                    'transition duration-200',
                    'dark:text-white/80', // Dark Mode
                    {
                        'text-blue-700 bg-blue-50 dark:text-white/80 dark:bg-blue-300': !context.focused && context.selected,
